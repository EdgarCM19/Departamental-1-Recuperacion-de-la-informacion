<template>
  ...
  <div class="action-buttons">
    <Button :on_click_fn="analisisLexico" :disabled="activeButtons.lexico"
      >Analisis Lexico</Button
    >
    <Button :on_click_fn="eliminacion" :disabled="activeButtons.cerradas"
      >Eliminacion de palabras</Button
    >
    <Button :on_click_fn="truncamiento" :disabled="activeButtons.truncamiento"
      >Truncamiento</Button
    >
  </div>
  <!-- <div>{{text}}</div> -->
  ...
  setup() {
    ...
    function analisisLexico() {
      if (text.value.length <= 0) {
        alert(
          "Seleccione un documento antes de tratar de hacer el analisis lexico"
        );
        return;
      }
      // const re = new RegExp('[\.\,\:\;\?\!\'\"\”\“\’\…\(\)\[\]\{\}\-\-]|[0-9]+', 'g'); // eslint-disable-line
      const re = new RegExp("[0-9]+", "g"); // eslint-disable-line
      const puntuation = [
        ".",
        ",",
    ...
    function eliminacion() {
      ...
      if (activeButtons.lexico === false) {
        alert(
          "Ejecute primero el analisis lexico antes de la eliminacion de palabras"
        );
        return;
      }
    ...
    function truncamiento() {
      ...
      if (activeButtons.lexico === false || activeButtons.cerradas === false) {
        alert(
          "Ejecute primero el analisis lexico y la eliminacion de palabras antes del truncamiento"
        );
        return;
      }
